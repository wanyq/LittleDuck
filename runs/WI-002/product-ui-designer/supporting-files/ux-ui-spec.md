...
版本：WI-002 revision 6（完整重做）

依据：`LittleDuck_Chatbot_MVP_PRD.md` V1.7、`页面UI稿/用户端UI` 五张用户端正式稿、`页面UI稿/管理端UI` 六张管理端正式稿
...
- 保留正式稿的鸭子 Logo、绿色品牌主色、大面积白色/浅灰背景、高圆角、轻边框、高留白和黑灰文字；不得用抽象占位 Logo 替换正式品牌图形，也不得合并两端明确不同的标识。H5 最终补稿使用 `littleduck-logo.svg`（双眼正脸、圆形白脸、居中鸭嘴、羽冠），管理端使用 `littleduck-admin-logo.svg`（单眼侧身整鸭、身体、翅膀和双脚）。
- 用户端继续使用右侧绿色用户气泡、左侧头像 + 白色助手气泡。
- 登录/注册页继续使用居中品牌区、大尺寸输入框、绿色主按钮。
- 新增的加载、错误、停止和重试状态使用克制的内联反馈，不引入大面积弹窗或新的视觉体系。
- 管理端严格继承正式稿的浅灰画布、白色固定侧栏、顶部品牌区、双菜单、底部管理员信息、面包屑、低阴影白卡、表格和步骤轴，不另造信息架构。

### 1.2 正式输入与补稿关系

- 5 张用户端正式稿定义认证、聊天、键盘和抽屉的基准画面；缺失状态在同一 H5 视觉语言下补齐。
- 5 张管理端页面稿定义管理员登录、配置、列表、聊天详情和调用详情；`组件补充稿.png` 定义骨架、空态、失败、按钮、校验、Toast、折叠和复制反馈。
- 11 张输入逐项映射见 `input-mapping.md`；所有 PRD 冲突与取舍见 `design-decisions.md`。
...
| Token | 建议值 | 用途 |
| --- | --- | --- |
| `--brand-600` | `#31B58B` | 主按钮、发送、选中、链接；与正式稿绿色一致 |
| `--brand-700` | `#24966F` | 按压/悬停 |
| `--brand-100` | `#E8F6F0` | 当前会话、轻提示背景 |
| `--page-bg` | `#F7F8FA` | 聊天页、管理端内容背景 |
...
- H5 输入框/按钮：16–24 px；消息气泡：18–22 px；状态小胶囊：999 px。
- 管理端输入框/按钮：6–8 px；卡片：12–14 px；表格容器：12 px。
- H5 只在键盘态底部输入容器和抽屉使用极轻阴影；管理端卡片使用 `0 2px 8px rgba(15,23,42,.06)` 与浅边框。
...
| 左右页边距 | 认证页 32 px；聊天页 16 px；430 px 时各增加 4 px |
...
- 高度：64 px + 顶部安全区；与 1088 × 1936 正式稿的白色导航比例一致。
- `position: sticky; top: 0`，层级高于消息区、低于抽屉。
- 左侧正式鸭子 Logo 40 × 40 px；中间标题 `LittleDuck`；右侧历史入口 44 × 44 px。
...
- 正常态高度：72 px + 底部安全区；多行时随输入框增长，最大约 160 px + 安全区。
...
| --- | --- | --- |
| 左侧导航 | 240 px 固定 | 260–276 px 固定，以正式稿比例为上限 |
| 内容区 | 剩余宽度，左右 28 px | 剩余宽度，左右 32–36 px |
| 顶部页头 | 72 px | 72 px |
| 页面背景 | `#F8F8FA` | 同左 |
...
- 全屏浅灰背景，中间 576 px 宽、约 765 px 高的白色低阴影卡片；1280 px 高度不足时允许卡片自然滚动并保留 32 px 上下安全留白。
- 品牌 Logo 144 px、标题 `LittleDuck 管理后台`、副标题 `登录后管理模型配置与聊天记录`，均按正式登录稿居中。
- 账号、密码输入框和全宽主按钮。
- 密码框保留正式稿的眼睛图标，用于本字段显隐；该交互不影响 API Key 必须明文展示的规则。
- 不提供注册、找回密码、修改密码入口。
...
- 主表单卡片占内容区可用宽度，1440 px 基准下约 970–1018 px；字段单列排列，保持正式稿的大留白和底部右对齐按钮组。
- 服务商：只读输入 `OpenAI`。
- API Key：多行或单行可横向滚动的明文输入；不提供显示/隐藏切换，避免暗示默认脱敏。视觉稿、测试数据和文档示例只使用明确标注为假的完整占位值，不使用真实凭据。
- 模型：普通文本输入。
- 操作区：左侧显示 `测试连接会发送最小测试请求，可能产生少量模型费用`，右侧次按钮 `测试连接`、主按钮 `保存配置`。
...
- 正序展示全部消息。
- 采用正式稿的全宽记录行，不使用 H5 左右聊天气泡：用户行为浅绿色底，助手记录白底浅边框。
- 每条左侧为头像 + 角色标签，中间为正文，右侧为时间与状态；代码块位于助手正文内并延伸到可读宽度。
- 每条显示角色、时间、状态。
- 状态徽标：成功、发送失败、进行中、失败、已停止。
- 重试产生的多个助手记录分别排列；不得覆盖。
- 管理端允许对消息正文复制；正文复制按钮在 hover/focus 显示，代码块复制按钮固定在右上，键盘仍可访问。
...
- 按调用时间正序。
- 采用正式稿的左侧绿色步骤轴与序号圆点，右侧为可折叠白色调用卡片。
- 卡片摘要行依次为 `步骤 N`、调用类型、状态、调用时间、服务商、模型、关联内容、展开/收起箭头；在 1280 px 时允许摘要字段换行但不隐藏。
