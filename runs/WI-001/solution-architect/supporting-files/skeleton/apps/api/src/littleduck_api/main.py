...
from .schemas import CreateGenerationRequest
...
def create_app(
    ...
    repository = GenerationRepository(database.sessions)
    service = GenerationService(repository, engine or DemoGenerationEngine())

    @asynccontextmanager
    async def lifespan(_: FastAPI) -> AsyncIterator[None]:
        await repository.fail_interrupted_generations()
        yield
    ...
    async def current_user_id(
        ...
        user_id = await repository.authenticate_user(ld_user_session)
        if user_id is None:
            raise PermissionError
        return user_id
    ...
    async def get_generation(
        ...
        return await repository.get_generation(generationId, user_id)
    ...
    async def stop_generation(
        ...
        view = await service.request_stop(generationId, user_id)
