...
`sequence` 只用于单条活连接内检查顺序，从 1 递增；它不持久化，也不能用于断线重放。
...
零到多个。`delta` 只能追加；`accumulatedLength` 用于发现客户端渲染异常，不用于恢复。
...
唯一停止终态。包含权威 generation、assistantMessage 和 `stoppedBy=user|logout`。停止不删除
已生成部分内容。
...
MVP 不续接供应商流。启动时把遗留 streaming 状态写为 failed/
`GENERATION_INTERRUPTED`，保留部分内容并允许重试。服务器服务恢复与单次模型流续接是两项
不同承诺，Mission 只要求前者。
