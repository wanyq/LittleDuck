...
5. [`supporting-files/littleduck-logo.svg`](supporting-files/littleduck-logo.svg)：H5 正脸鸭头矢量资产，保留双眼、圆形白脸、居中鸭嘴与羽冠。
6. [`supporting-files/littleduck-admin-logo.svg`](supporting-files/littleduck-admin-logo.svg)：管理端侧身整鸭矢量资产，保留单眼侧脸、身体、翅膀、鸭嘴和双脚。
7. [`supporting-files/h5-visual-board.html`](supporting-files/h5-visual-board.html)：H5 高保真基准与最终状态补稿源码。
8. [`supporting-files/h5-visual-board.png`](supporting-files/h5-visual-board.png)：1680 × 3200 Chrome 渲染结果。
9. [`supporting-files/admin-visual-board.html`](supporting-files/admin-visual-board.html)：PC 管理端高保真基准、最终页面和状态矩阵源码。
10. [`supporting-files/admin-visual-board.png`](supporting-files/admin-visual-board.png)：1800 × 5900 Chrome 渲染结果。
11. [`supporting-files/scripts/render-boards.sh`](supporting-files/scripts/render-boards.sh)：可重复执行的两板渲染脚本。
12. [`supporting-files/scripts/validate-delivery.sh`](supporting-files/scripts/validate-delivery.sh)：输入映射、内容、尺寸、分端正式 Logo 使用与凭据模式自动检查。
13. [`supporting-files/self-test.md`](supporting-files/self-test.md)：环境、步骤、自动结果、人工视觉检查和已知限制。
...
- H5 注册/导航/欢迎/助手头像/抽屉只使用正脸 `littleduck-logo.svg`；管理端登录/侧栏只使用整鸭 `littleduck-admin-logo.svg`，两端不再混用或使用抽象 CSS 圆环；
